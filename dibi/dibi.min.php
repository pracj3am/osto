...
new
InvalidArgumentException('Configuration must be array, string or ArrayObject.');}self::alias($config,'username','user');self::alias($config,'password','pass');self::alias($config,'host','hostname');if(!isset($config['driver'])){$config['driver']=dibi::$defaultDriver;}$driver=preg_replace('#[^a-z0-9_]#','_',$config['driver']);$class="Dibi".$driver."Driver";if(!class_exists($class,FALSE)){ include_once __DIR__."/../drivers/$driver.php";if(!class_exists($class,FALSE)){throw
new
...
function
asDate($key,$format=NULL){$time=$this[$key];if((int)$time===0){return
NULL;}elseif($format===NULL){return
is_numeric($time)?(int)$time:strtotime($time);}elseif($format===TRUE){return
new
DateTime53(is_numeric($time)?date('Y-m-d H:i:s',$time):$time);}elseif(is_numeric($time)){return
date($format,$time);}else{$time=new
DateTime53($time);return$time->format($format);}}public
function
...
as$k=>$v){if(is_string($k)){$pair=explode('%',$k,2);$vx[]=$this->formatValue($pair[0],isset($pair[1])?$pair[1]:'n').(empty($v)?'':' AS '.$v);}else{$pair=explode('%',$v,2);$vx[]=$this->delimite($pair[0]);}}return
...
as$v){$vx[]=$this->formatValue($v,$modifier);}return$vx?implode(', ',$vx):'NULL';}}if($modifier){if($value!==NULL&&!is_scalar($value)&&!($value
instanceof
...
REVISION='e177436 released on 2010-01-14';const
